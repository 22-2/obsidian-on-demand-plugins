import { PluginMode, PLUGIN_MODE, PluginModes } from "../../core/types";
...
export class MaintenanceService {
    ...
    async buildSyncPreview(
        ...
        if (direction === "coreToLazy") {
            const toEnable = manifests.filter(
                (m) =>
                    onDisk.has(m.id) &&
                    this.ctx.getPluginMode(m.id) === PLUGIN_MODE.ALWAYS_DISABLED
            );
            const toDisable = manifests.filter(
                (m) =>
                    !onDisk.has(m.id) &&
                    this.ctx.getPluginMode(m.id) === PLUGIN_MODE.ALWAYS_ENABLED
            );
    ...
    private async syncLazyToCore(): Promise<SyncResult> {
        ...
        const currentOnDisk = Array.from(
            this.registry.enabledPluginsFromDisk
        );
        const isSame =
    ...
    private getAlwaysEnabledIds(): string[] {
        const ids = this.ctx.getManifests()
