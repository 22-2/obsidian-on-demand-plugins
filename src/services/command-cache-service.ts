...
export class CommandCacheService {
  ...
  async getCommandsForPlugin(pluginId: string): Promise<CachedCommand[]> {
    const wasEnabled = this.deps.obsidianPlugins.enabledPlugins.has(pluginId);
    if (!wasEnabled) {
    ...
    const pluginCommands = commands
      .filter((command) => this.deps.getCommandPluginId(command.id) === pluginId)
      .map((command) => ({
        id: command.id,
        name: command.name,
        icon: command.icon,
    ...
    if (!wasEnabled && this.deps.getPluginMode(pluginId) !== "keepEnabled") {
  ...
  isCommandCacheValid(pluginId: string): boolean {
    ...
    const cachedVersion = this.deps.getData().commandCacheVersions?.[pluginId];
