...
export class ToolsModal extends Modal {
    ...
    private activeTabId: string = "maintenance";
    private batchOperation: "sync" | "replace" = "sync";
    ...
    onOpen() {
        ...
        this.createTab(tabHeader, "batch", "Batch", "refresh-cw");
    ...
    private renderActiveTab() {
        ...
        switch (this.activeTabId) {
            case "maintenance":
                ...
                break;
            case "batch":
                this.buildBatchSection(this.tabContentEl);
                break;
            case "debug":
    ...
    private buildBatchSection(container: HTMLElement) {
        new Setting(container).setName("Batch operations").setHeading();

        new Setting(container)
            .setName("Operation")
            .setDesc("Choose between syncing settings or batch replacing plugin modes.")
            .addDropdown((dropdown) => {
                dropdown
                    .addOption("sync", "Sync settings")
                    .addOption("replace", "Replace modes")
                    .setValue(this.batchOperation)
                    .onChange((value: string) => {
                        this.batchOperation = value as "sync" | "replace";
                        this.renderActiveTab();
                    });
            });

        if (this.batchOperation === "sync") {
            this.buildSyncContent(container);
        } else {
            this.buildReplaceContent(container);
        }
    }

    private buildSyncContent(container: HTMLElement) {
    ...
    private buildReplaceContent(container: HTMLElement) {
        ...
        new Setting(container).setClass("lazy-replace-setting").addButton((btn) =>
            btn
