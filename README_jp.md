...
このプラグインはプラグインを一括で切り替える際に、Vault 内の `.obsidian/community-plugins.json` を直接書き換えることがあります。設定が変更されるたびに、このプラグインのフォルダ以下へ `community-plugins.json` と `data.json` のバックアップが最大 3 つ自動作成されます。さらに、プラグインをインストールした直後に、変更・削除されないバックアップも 1 つ作成されます。それでも念のため、ご自身でもバックアップを取っておいてください。
...
### 定期実行やフックを使用するプラグイン
以下を利用するプラグインは **Lazy on layout ready** での読み込みを推奨します：
...
これらのプラグインを **Lazy on demand** に設定すると、トリガーされるまで有効化されないため、バックグラウンド処理やフックが期待通りに動かない可能性があります。
